...
// Para crear: basta con ser miembro (u owner) del viaje del día
export const checkTripMemberForActivity = async (req: CustomRequestActivities, res: Response, next: NextFunction) => {
    try {
...
export const checkActivityOwnerOrCreator = async (req: CustomRequestActivities, res: Response, next: NextFunction) => {
    try {
        const { id } = req.params;
        ...
        const isCreator = activity.createdBy?.toString() === req.user?.id;
